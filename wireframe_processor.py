...
# Use Gemini API key instead of OpenA
        ...
        for component_type, rects in ui_components.items():
            if component_type in colors:
                for x, y, w, h in rects:
                    cv2.rectangle(
                        viz_img, (x, y), (x + w, y + h), colors[component_type], 2
                    )

        cv2.imwrite("detected_components.png", viz_img)
        print(
            "Component detection complete. Visualization saved to 'detected_components.png'"
        )
...
def extract_code_block(text: str, file_type: str) -> str:
    """
    Extracts the CSS code block from a given string.

    Args:
        text (str): The input text containing a CSS code block.

    Returns:
        str: Extracted CSS code or an empty string if no match is found.
    """
    if file_type == "css":
...
def wireframe_to_code(image_path):
    ...
    try:
        ...
        wireframe_analysis_task, html_generation_task, css_generation_task = (
            create_tasks(ui_analyzer, html_developer, css_stylist, wireframe_data)
        )
        ...
        # Verify results
        html = extract_code_block(result.tasks_output[1].raw, file_type="html")
        css = extract_code_block(result.tasks_output[2].raw, file_type="css")
        if css:
            print("css found....")
            css_content = css
        ...
        return {"html": html_content, "css": css_content, "analysis": analysis_content}
...
if __name__ == "__main__":
    try:
        ...
        print(
            f"Analysis saved to 'analysis.json' ({len(result['analysis'])} characters)"
        )
        ...
        try:
            ...
            plt.subplot(1, 3, 3)
            plt.imshow(
                cv2.cvtColor(cv2.imread("detected_components.png"), cv2.COLOR_BGR2RGB)
            )
